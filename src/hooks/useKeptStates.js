...
export function useKeptStates() {
  ...
  const addKeptState = useCallback((stateId, displayName) => {
    ...
    setKeptStates(currentStates => {
      ...
      const updatedStates = currentStates.length >= MAX_KEPT_STATES
  ...
  const removeKeptState = useCallback((stateId) => {
  ...
  const getKeptStates = useCallback(() => {
    ...
    return [...keptStates].sort((a, b) =>
      new Date(b.dateAdded) - new Date(a.dateAdded)
    );
  }, [keptStates]);

  const hasKeptState = useCallback((stateId) => {
