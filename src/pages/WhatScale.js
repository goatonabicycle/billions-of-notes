	...
	const renderScaleNotes = (scaleNotes, inputNotes) => {
		...
		const renderedNotes = scaleNotes.map((scaleNote) => {
			...
			if (inputNotes.includes(sharpNote)) {
				return (
					<span className="px-1 border border-primary-500 text-primary-400 font-semibold" key={sharpNote}>
						{sharpNote}
					</span>
				);
			}
					...
					<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
						{detectedScales.map((scaleName, index) => {
							...
							return (
								...
								>
									<div className="font-semibold text-primary-400 mb-2">{scaleName}</div>
