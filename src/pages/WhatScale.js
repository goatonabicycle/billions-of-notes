...
export default function WhatScale() {
	...
	const renderScaleNotes = (scaleNotes, inputNotes) => {
		...
		const renderedNotes = scaleNotes.map((scaleNote) => {
			...
			return <span className="px-1" key={sharpNote}>{sharpNote}</span>;
		});
	...
	return (
		<div className="flex flex-col h-screen gap-6 p-6 pt-6">
			...
			{detectedScales.length > 0 && (
				<div className="flex-grow bg-gray-900/80 backdrop-blur-sm border border-pink-500/20 p-4 rounded-lg overflow-auto">
					<h3 className="text-lg font-semibold text-gray-200 mb-4">Detected Scales:</h3>
					<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
						{detectedScales.map((scaleName, index) => {
							...
							return (
								...
								>
									<div className="font-semibold text-pink-400 mb-2">{scaleName}</div>
